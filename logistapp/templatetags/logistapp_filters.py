...
@register.filter('get_stock_name')
def get_stock_name(index):
    cont = ContractHandler()
    name = cont.get_stock_name(index)
    if name:
        return name
    return ''
...
def get_date(time):
    date = datetime.date.fromtimestamp(time).strftime('%Y-%m-%d')
    return date


@register.filter('get_stock_address')
def get_stock_address(index):
    cont = ContractHandler()
    address = cont.get_stock_address(index)
    if address:
        return address
    return ''
